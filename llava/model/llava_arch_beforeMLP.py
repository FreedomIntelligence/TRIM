...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        for i in range(bsz):
            ...
            cur_image_features_pca, actual_dim = self.apply_pca(cur_image_features)
        ...
        image_features_pca_padded = torch.stack(pca_image_features)

        # Apply mm_projector
        image_features = self.get_model().mm_projector(image_features_pca_padded)
        return image_features, actual_dims

    # Define the PCA function with autonomous dimension selection
    def apply_pca(self, cur_image_features, variance_threshold=0.99):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features, actual_dims = self.encode_images(concat_images)
            ...
            image_features, actual_dims = self.encode_images(images)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features[:actual_dims[cur_image_idx-1]])  # Remove padding
                    cur_new_labels.append(torch.full((actual_dims[cur_image_idx-1],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
