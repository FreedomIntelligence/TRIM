...
def parse_args():
    ...
    parser.add_argument("--result_file", required=True, help="The path to save result score.")
...
def annotate(prediction_set, caption_files, output_dir):
    ...
    for file in tqdm(caption_files):
        ...
        try:
            ...
            FOWARD_URL = f"{openai.api_base}/chat/completions"
            TIME_OUT = 20
...
def main():
    ...
    for key, result in combined_contents.items():
        ...
        try:
            ...
            try:
                score_match = result[0]['score1']
            except:
                # print(result)
                score_match = result[0]['final']
    ...
    ds_name = 'MSVD' if 'msvd' in args.pred_path.lower() else 'MSRVTT'
    with open(args.result_file, 'a') as f:
        f.write(f"{ds_name} (Total Acc)\t{round(accuracy*100,2)}\n")
        f.write(f"{ds_name} (Average score)\t{round(average_score,2)}\n")
